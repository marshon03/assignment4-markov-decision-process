...
from constants import TERM_STATE_MAP, GOAL_STATE_MAP, FL8x8, FL20x20
...
def visualize_env(env, env_name, title=None):
    ...
    for i in range(M):
        for j in range(N):
            if (N * i + j) in TERM_STATE_MAP[getEnvName(env_name)]:
                arr[i, j] = 0.25
            elif (N * i + j) in GOAL_STATE_MAP[getEnvName(env_name)]:
                arr[i, j] = 1.0
...
def render_policy(pi, env_name, shape):
    ...
    for index in TERM_STATE_MAP[getEnvName(env_name)]:
        actions[index] = 999
    for index in GOAL_STATE_MAP[getEnvName(env_name)]:
        actions[index] = 1000
...
def getEnvName(env_name):
    envname = ''
    if env_name == 'FrozenLake8x8-v1':
        envname = FL8x8
    elif env_name == 'FrozenLake20x20-v1':
       envname = FL20x20
    elif env_name == '':
        envname = ''
